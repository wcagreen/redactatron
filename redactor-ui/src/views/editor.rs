...
impl EditorPage {
    ...
    fn render_sidebar(&mut self, ui: &mut egui::Ui) {
        ...
        let redactions_for_current = self.redaction_areas.iter()
        ...
        let mode_button = if self.redaction_mode {
            egui::Button::new("✏️ Redaction Mode: ON")
                .fill(egui::Color32::from_rgb(220, 50, 50))
        } else {
            egui::Button::new("✏️ Redaction Mode: OFF")
        ...
        if self.redaction_mode {
            ui.colored_label(egui::Color32::LIGHT_YELLOW, "Left-click drag to create redaction");
        } else {
    ...
    fn render_document_content(&mut self, ui: &mut egui::Ui, file_path: &PathBuf, ctx: &egui::Context) {
        ...
        if let Some(texture) = self.texture_cache.get(file_path) {
            ...
            ui.vertical(|ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.separator();
                    if self.redaction_mode {
                        ui.colored_label(egui::Color32::LIGHT_RED, "🖍️ REDACTION MODE - Drag to create boxes");
                    } else {
    ...
    fn redact_and_export_image(&self, current_file: &PathBuf) {
        ...
        let redactions: Vec<_> = self.redaction_areas.iter()
            .filter(|r| &r.file_path == current_file)
            .collect();
        
        if redactions.is_empty() {
            println!("No redactions to apply");
            return;
        }
        ...
        if let Some(save_path) = rfd::FileDialog::new()
            .set_file_name(format!("{}_redacted.png", 
            ...
            .save_file()
        {
            match img_rgba.save(&save_path) {
                Ok(_) => println!("Redacted image saved to: {}", save_path.display()),
                Err(e) => println!("Failed to save redacted image: {}", e),
            }
        }
    }

    fn export_redacted_file(&self) {
        let current_file = &self.files[self.current_file_index];
        
        // Check if current file is an image
        let extension = current_file.extension()
            .and_then(|e| e.to_str())
            .unwrap_or("")
            .to_lowercase();
